...
export const Header: React.FC<HeaderProps> = ({ activeTab, onNavigateTab }) => {
  ...
  const highlightedItems = [
    { id: 'unidades', label: 'Unidades', badge: 'São Paulo & +', customClass: 'border-adk-yellow text-adk-yellow hover:bg-adk-yellow hover:text-zinc-950 hover:shadow-cyan-400' },
    { id: 'atletas', label: 'Atletas', badge: 'Grand Slam', customClass: 'bg-adk-yellow text-zinc-950 hover:bg-white tracking-widest' },
  ];
  ...
  return (
    <header className="sticky top-0 w-full bg-adk-dark/95 backdrop-blur-md border-b border-zinc-800/80 z-50 transition-all duration-300" id="adk-main-header">
      {/* Top microbar for contact info, hours, and live alerts */}
      <div className="bg-zinc-950 border-b border-zinc-900 text-[11px] text-zinc-400 py-2 px-4 shadow-inner" id="header-topbar">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div className="flex items-center space-x-3">
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-green-500/10 text-green-400 border border-green-500/25">
              ● VAVAS ABERTAS INTENSIVO DE JULHO
            </span>
            <button 
              onClick={() => handleTabClick('programas')}
              className="text-adk-yellow hover:text-white transition-colors duration-150 font-mono font-bold tracking-tight uppercase"
            >
              Matrículas 2026
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        >
          <div className="w-12 h-12 bg-adk-yellow flex items-center justify-center rounded-sm font-black text-adk-dark text-xl transition-all duration-300 shadow-md group-hover:scale-105">
            ADK
          </div>
          <div className="flex flex-col">
            <h1 className="text-lg font-bold tracking-tight leading-none uppercase text-white group-hover:text-adk-yellow transition-colors">ADK Tennis</h1>
            <p className="text-[10px] text-adk-yellow uppercase tracking-[0.2em] font-medium leading-none mt-1">High Performance Center</p>
          </div>
        </button>
        ...
        <div className="hidden lg:flex items-center space-x-3" id="highlighted-links">
            ...
            return (
              <button
                ...
                className={`relative px-4 py-2 text-xs font-black uppercase tracking-wider rounded border transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer flex items-center gap-1.5 ${hl.customClass} ${
                  isActive ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-950' : ''
                }`}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-zinc-950 border-t border-zinc-850 py-4 px-4 space-y-3 shadow-2xl animate-fade-in" id="mobile-drawer">
          <div className="grid grid-cols-2 gap-2 pb-3 border-b border-zinc-850">
            {highlightedItems.map((hl) => (
              ...
              >
                <span>{hl.label}</span>
                <span className="text-[7.5px] font-mono opacity-80 uppercase scale-90">{hl.badge}</span>
              </button>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2.5 text-sm font-bold uppercase tracking-wide text-left rounded transition-colors ${
                    isActive ? 'text-zinc-950 bg-adk-yellow' : 'text-white hover:text-adk-yellow hover:bg-zinc-900/50'
                  }`}
